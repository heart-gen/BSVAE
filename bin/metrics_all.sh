...
IFS=' ' read -r -a SECTIONS <<< "${BSVAE_EVAL_SECTIONS:-VAE_genenet beta_genenet}"
if [[ ${#SECTIONS[@]} -eq 0 ]]; then
    echo "No config sections supplied via BSVAE_EVAL_SECTIONS." >&2
    exit 1
fi

LOGGER="${REPO_ROOT}/metrics_all.out"
: > "${LOGGER}"

for section in "${SECTIONS[@]}"; do
    exp_name="${BSVAE_EXPERIMENT_PREFIX:-}${section}"
    result_dir="${REPO_ROOT}/results/${exp_name}"
    {
        echo
        echo "Evaluating section: ${section}";
        echo "Experiment name: ${exp_name}";
    } >> "${LOGGER}"

    if [[ ! -d "${result_dir}" ]]; then
        msg="Skipping ${exp_name}: results directory not found."
        echo "${msg}" >> "${LOGGER}"
        echo "${msg}" >&2
        continue
    fi
